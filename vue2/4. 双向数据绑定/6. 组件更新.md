...
对于新旧节点不同的情况，这种`创建新节点 -> 更新占位符节点 -> 删除旧节点`的逻辑是很容易理解的。还有一种组件 vnode 的更新情况是新旧节点相同，它会调用 **patchVNode** 方法，它的定义在 `src/core/vdom/patch.js` 中：
...
![第二步](../../images/vue2-patch2.png)

第三步：

![第三步](../../images/vue2-patch3.png)

第四步：

![第四步](../../images/vue2-patch4.png)

第五步：

![第五步](../../images/vue2-patch5.png)

第六步：

![第六步](../../images/vue2-patch6.png)
...
（1）使用旧列表的头一个节点 oldStartVnode 与新列表的头一个节点 newStartVnode 对比
...
-   当旧列表的头一个节点 oldStartVnode 与新列表的最后一个节点 newEndVnode 对比时 key 相同。那么旧列表的头指针 oldStartIdx 向后移动一位；新列表的尾指针 newEndIdx 向前移动一位。同时移动 dom：oldStartVnode.elm 移动到 newEndVnode.elm 之后。

-   当旧列表的最后一个节点 oldEndVnode 与新列表的头一个节点 newStartVnode 对比时 key 相同。那么旧列表的尾指针 oldEndIdx 向前移动一位；新列表的头指针 newStartIdx 向后移动一位。同时移动 dom：oldEndVnode.elm 移动到 oldStartVnode.elm 之前。
...
-   在旧列表中找到了，将找到的节点和 newStartVnode 比较
    ...
    -   相同则执行 patchVnode，并将找到到节点对应到 dom 移动到 oldStartVnode 对应的 dom 之前;
